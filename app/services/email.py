...
def send_contact_email_message(subject, email, content, ip, user_id):
    """
    Отправка письма с отзывом
    """
    user = User.objects.get(id=user_id) if user_id else None
    message = render_to_string('user_profile/email/feedback_email_send.html', {
        'email': email,
        'content': content,
        'ip': ip,
        'user': user,
...
def send_activate_email_message(user_id):
    """
    Отправка письма с подтверждением для аккаунта
    """
    ...
    message = render_to_string('user_profile/email/activate_email_send.html', {
        'user': user,
